  ...
  .handler(async ({ data, context }): Promise<PetriRunDetail | null> => {
    ...
    return {
      run: {
        ...
        input_source_map: run.input_source_map ?? {},
        data_completeness: run.data_completeness ?? { score: 0 },
        fallbacks: run.fallbacks ?? null,
        abstention_reasons: run.abstention_reasons ?? null,
